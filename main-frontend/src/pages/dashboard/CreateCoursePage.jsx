...
export default function CreateCoursePage() {
  ...
  const submit = async e => {
    ...
    try {
      await createCourse(title, desc, sessionTiming)
    ...
    <div style={{ maxWidth: 620 }} className="fade-up">
      ...
      <div className="card" style={{ padding: 28 }}>
        <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            <label className="label">Session Timing</label>
            <input
              className="input"
              value={sessionTiming}
              onChange={e => setSessionTiming(e.target.value)}
              placeholder="e.g. Mon & Wed, 7:00 PM – 8:30 PM (GMT+5:30)"
            />
            <p style={{ fontSize: 11.5, color: 'var(--text2)', marginTop: 5, opacity: 0.7 }}>
              Let students know when you plan to hold live sessions.
            </p>
          </div>
