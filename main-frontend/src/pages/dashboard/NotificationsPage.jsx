...
import { getNotifications, markRead, deleteNotif, testNotification } from '@/api'
...
const TYPE_COLOR = {
  ...
  session_start:    { bg: 'rgba(16,185,129,0.08)', border: 'rgba(16,185,129,0.15)', dot: '#10b981' },
  teacher_approved: { bg: 'rgba(0,229,160,0.08)', border: 'rgba(0,229,160,0.15)', dot: '#00e5a0' },
  teacher_rejected: { bg: 'rgba(255,77,109,0.08)', border: 'rgba(255,77,109,0.15)', dot: '#ff4d6d' },
  admin_message:    { bg: 'rgba(168,85,247,0.08)', border: 'rgba(168,85,247,0.15)', dot: '#a855f7' },
  admin_broadcast:  { bg: 'rgba(168,85,247,0.08)', border: 'rgba(168,85,247,0.15)', dot: '#a855f7' },
}
...
export default function NotificationsPage() {
  ...
  const load = () => getNotifications().then(r => setNotifs(r.data || [])).catch(() => {}).finally(() => setLoading(false))
  ...
  const sendTest = async () => {
    try {
      ...
      toast('Failed: ' + (e.response?.data?.detail || e.message), 'error')
    }
  }
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto' }} className="fade-up">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        ...
        <button className="btn btn-ghost btn-sm" onClick={sendTest} style={{ marginLeft: 8, color: '#818cf8' }}>Test Notification</button>
      </div>
